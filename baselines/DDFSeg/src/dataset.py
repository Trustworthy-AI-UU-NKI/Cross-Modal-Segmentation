...
class MMWHS(Dataset):
  ...
  def __getitem__(self, index):
    ...
    elif self.dataset_size == self.source_size:
...
class CHAOS(Dataset):
  def __init__(self, args, labels, fold_target, fold_source):
    ...
    self.all_source_images = sorted(glob.glob(os.path.join(self.data_dir1, "images/case_*")))
    self.all_source_labels = sorted(glob.glob(os.path.join(self.data_dir1, "labels/case_*")))
    self.all_target_images = sorted(glob.glob(os.path.join(self.data_dir2, "images/case_*")))
    self.all_target_labels = sorted(glob.glob(os.path.join(self.data_dir2, "labels/case_*")))
  ...
  def __getitem__(self, index):
    if self.source_size == self.target_size:
      image_s = self.transform_img(self.images_source[index]).unsqueeze(0)
      label_s = self.transforms_seg(self.labels_source[index]).unsqueeze(0)
     
      image_t = self.transform_img(self.images_target[index]).unsqueeze(0)
      label_t = self.transforms_seg(self.labels_target[index]).unsqueeze(0)
      ...
      image_t = self.transform_img(self.images_target[random_index]).unsqueeze(0)
      label_t = self.transforms_seg(self.labels_target[random_index]).unsqueeze(0)
    else:
      random_index = random.randint(0, self.source_size - 1)
     
      image_s = self.transform_img(self.images_source[random_index]).unsqueeze(0)
      label_s = self.transforms_seg(self.labels_source[random_index]).unsqueeze(0)
...
class CHAOS_single(Dataset):
  ...
  def __getitem__(self, index):
    image = self.transform_img(self.images[index]).unsqueeze(0)
    label = self.transforms_seg(self.labels[index]).unsqueeze(0)
    return torch.cat((image, image, image), dim=0), label
