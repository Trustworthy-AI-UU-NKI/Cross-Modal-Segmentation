...
def evaluate_unet(val_loader, model, device, epoch, dice_metric, best_metric, dir_checkpoint, writer):
    ...
    with torch.no_grad():
        for val_data in val_loader:
            ...
            val_labels_onehot =  MapLabelValue(orig_labels=[1, 2, 3, 4, 5 , 6 , 7], target_labels=[1, 1, 1, 1, 1 , 1 , 1])(val_labels)
        ...
        if metric < best_metric:
            ...
            print(f"saved new best metric modelat {epoch}")
...
def main(args):
    ...
    filename = f'LR_{args.lr}_BS_{args.bs}_modality_{args.modality}'
    ...
    val_loader = DataLoader(val_ds, batch_size=args.bs, shuffle=True, num_workers=4, collate_fn = list_data_collate, pin_memory=torch.cuda.is_available())
    ...
    for epoch in range(args.epochs):
        ...
        epoch_loss = 0
        step = 0
        with tqdm(total=len(train_images), desc=f'Epoch {epoch + 1}/{epoch}') as pbar:
